...
class Vision_Embedding(nn.Module):
    def __init__(self, config: Dict):
        super().__init__()
        self.backbone = AutoModel.from_pretrained(config["vision_emmbedding"]["image_encoder"])
        # freeze all parameters of pretrained model
        for param in self.backbone.parameters():
            param.requires_grad = False
            
        self.proj = nn.Linear(config["vision_emmbedding"]['d_feature'], config["vision_emmbedding"]['d_model'])
        self.gelu = nn.GELU()
